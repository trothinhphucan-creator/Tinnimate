...
async function main() {
  ...
  const { data: modelRow } = await supabase.from('llm_models').select('*').eq('model_id', newModel).maybeSingle()
  ...
  if (!modelRow) {
    ...
    await supabase.from('llm_models').update({ is_active: true, provider: 'litellm' }).eq('model_id', newModel)
    console.log('✅ Updated')
  }
